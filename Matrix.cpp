...
int Matrix_row(const Matrix* mat, const int* ptr) {
  int r = ((ptr - (mat->data)) / Matrix_width(mat));
  return r;
}
...
const int* Matrix_at(const Matrix* mat, int row, int column) {
  const int* f = mat->data + (row * mat->width) + column;
  return f;
}
...
void Matrix_fill_border(Matrix* mat, int value) {
  for(int k = 0; k < Matrix_width(mat); k++){
    mat->data [k] = value;
    *Matrix_at(mat,Matrix_height(mat)-1,k) = value;
    *Matrix_at(mat,0,k) = value;

  }
  for(int i = 0; i < Matrix_height(mat) - 1; i++){
    mat->data [i] = value;
...
int Matrix_max(const Matrix* mat) {
  ...
  for (int i = 0; i < size; i++) {
    if (*(Matrix_at(mat,0,0)+i) > max ) 
      max = *(Matrix_at(mat,0,0)+i);
  }
  return max;
}
